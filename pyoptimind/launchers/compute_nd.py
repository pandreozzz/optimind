...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="Example: python compute_nd.py 2012 config.json /out/logdir 64 8",
    )
    parser.add_argument("year", type=int, help="Year to process")
    parser.add_argument("config_path", type=str, help="Path to configuration JSON")
    parser.add_argument("logdir", type=str, help="Path to output/log directory")
    parser.add_argument("total_mem_gb", type=float, help="Total memory available (GB)")
    parser.add_argument("nprocs", type=int, help="Number of Dask workers")
    return parser
...
def main() -> int:
    ...
    year = int(args.year)
    config_path = str(os.path.abspath(args.config_path))
    logdir = str(os.path.abspath(args.logdir))
    total_mem_gb = float(args.total_mem_gb)
    CONFIGDICT["nprocs"] = int(args.nprocs)
    ...
    cluster = LocalCluster(
        n_workers=CONFIGDICT["nprocs"],
        memory_limit=f"{(total_mem_gb * 0.95) / max(CONFIGDICT['nprocs'], 1):.2f}GB",
    )
    with Client(cluster):
        print("Client ready.")
