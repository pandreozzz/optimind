...
def copy_all_files(year, hourly=""):
    """
    ...
    Args:
        ...
        hourly: str - Hourly specification string (default: "")
    ...
    if CONFIGDICT["aerofromclimatology"]:
        ...
        cmd1 = f"{cpy_cmd} {CONFIGDICT['aerosolclimfile']} {os.environ['TMPDIR']}/fields/aerosol_cams_climatology.nc"
        print(cmd1)
        os.system(cmd1)
    else:
        print(f"{operation} fields from prognostics...")
        aero_pl_filename = os.path.join(AERO_DATADIR, aero_pl_namelike(year, CONFIGDICT["gridspec"])) + dataext
        aero_sfc_name = os.path.join(AERO_DATADIR, "data", aero_sfc_namelike(year, CONFIGDICT["gridspec"])) + dataext
    ...
    era5_ml_file = os.path.join(ERA5_DATADIR, f"era5_{year}_{CONFIGDICT['gridspec']}_{CONFIGDICT['hourly']}_{ERA5MLFILESIGN}{dataext}")
    era5_tend_ml_file = os.path.join(ERA5_DATADIR, f"era5_{year}_{CONFIGDICT['gridspec']}_{CONFIGDICT['hourly']}_{ERA5TENDFILESIGN}{dataext}")
    era5_sfc_file = os.path.join(ERA5_DATADIR, f"era5_{year}_{CONFIGDICT['gridspec']}_{CONFIGDICT['hourly']}_{ERA5SFCFILESIGN}{dataext}")

    for file_check in [era5_ml_file, era5_sfc_file]:
        if not os.path.exists(era5_ml_file):
            raise ValueError(f"{era5_ml_file} does not exist!")
    ...
    modis_nd_file = os.path.join(MODIS_DATADIR, f"modis_nd_nd13.monthlymeans_{year:4d}.AT.v1_{CONFIGDICT['gridspec']}.nc")
