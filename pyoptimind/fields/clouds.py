...
def get_cloudy_level(cum_tau_c, min_tot_tau_c, min_top_tau_c):
    ...
    representative_cloudy_level = cum_tau_c.where(
        is_cloud_layer_deep_enough
            ...
            dim="lev", skipna=True
            ).expand_dims(lev=[-1], axis=-1).astype(np.int16)
    return CloudyLevel(
        representative_cloudy_level.compute(),
        is_cloud_thick_enough.compute()
        )
...
def get_meteo_cloudy_slices(year : int, ifs_fields=None, cc_thresh : float = 0.8,
    ...
    time0 = time()
    this_ifs_layer = this_ifs[fld_list+new_fld_list].isel(
    ...
    if CONFIGDICT["aerofromclimatology"] and (CONFIGDICT["fixedaeromodellevel"] is not None):
        ...
        this_ifs_fixedlevel = this_ifs[["p", "t", "q", "rho_air"]].rename(lev="levaux").isel(
            levaux=cloud_base_level, drop=True
            ).where(total_mask)
        ...
        representative_aero_level = get_gros_aerolevel(
            cum_tau_c=cum_tau_c.where(ThisCloudyLevel.is_cloud_thick_enough, 0.)
            )
