"""Implements definitions related to IFS model levels"""

import numpy as np

from ..tools.constants import R_DRYAIR
...
def populate_mlfields(ds, keep_p_half : bool = False):
    ...
    print("Computing 3D pressure, rho_air, and level pressure difference. " +\
          f"minlev: {minlev}, maxlev: {maxlev}", flush=True)
    ...
    ds["p_half"] = (ds["sp"]*ds["hybi"].astype(np.float32).isel(nhyi=halfplevslice) +\
                   ds["hyai"].astype(np.float32).isel(nhyi=halfplevslice) \
                   ).rename(nhyi="half_lev").assign_coords(
